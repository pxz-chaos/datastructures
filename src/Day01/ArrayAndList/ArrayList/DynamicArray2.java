 ...
 */
public class DynamicArray2<E> implements List<E> {
    //成员变量
    ...
    private /*static 错误写法，静态不能访问非静态，所以这里的size就会找不到*/void rangeCheck(int index) {
        if (index < 0 || index >= size) {
            throw new IndexOutOfBoundsException("Index:" + index + ":Size:" + size + ",王德发，别越界了");
        }
    }
    ...
    public void add(E element) {
        if (size <= DEFAULT_CAPACITY) {
           ...
           /* elements[size] = element;
            ...
            //优化
            // elements[size++] = element;
            //再次优化，反正这个是最后一个位置添加元素，那我直接调用那个有索引的add方法
            add(size, element);
    ...
    public void add(int index, E element) {
        //需要对这个index进行合法性校验
        //注意，这里的index可以取得到size，因为size已经++了
        if (index < 0 || index > size) {
            throw new IndexOutOfBoundsException("Index:" + index + ":Size:" + size + ",王德发，别越界了");
        }
