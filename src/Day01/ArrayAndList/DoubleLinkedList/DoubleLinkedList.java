package Day01.ArrayAndList.DoubleLinkedList;
...
public class DoubleLinkedList<E> extends AbstractList<E> {
    ...
    public void add(int index, E element) {
        ...
        if (index == size) {//往最后面添加元素
            Node<E> oldLast = last;
            Node<E> newNode = new Node<>(oldLast/*getIndexNode(size-1)*/, element, null);
    ...
    public E remove(int index) {
        ...
        if (indexNode.prev == null) {//删除第一个节点
            first = indexNode.next;///直接让first指针指向下一个节点
        } else {
            indexNode.prev.next = indexNode.next;
        }

        //如果index=size-1，那么indexNode.next=null，null没有prev
        if (indexNode.next == null) {//删除最后一个节点
            last = indexNode.prev;//让last指针指向前一个节点
        } else {
